...
class LoggedInComponent: Component<LoggedInDependency> {
    ...
    @Singleton({ (self: LoggedInComponent) in LoggedStream(self.mutableScoreStream) })
    var loggedScoreStream: ScoreStream
...
The `@Singleton` and `@SingletonInstance` constructs in the example are property wrapped we provide (to be use only a `Component` class) that simply returns the same instance every time this `var` is accessed (as opposed to the one below it, which returns a new instance each time). This ties the lifecycle of this property to the lifecycle of the Component. `@Singleton` accepts a instantiation closure that requires no arguments, while `@SingletonInstance` accepts an instance of the type to be returned. The `@Singleton` property wrapper is useful when you want to create a new instance of a type that requires some dependencies from the component itself, while `@SingletonInstance` lets you bind dependencies provided by the `Component`.
